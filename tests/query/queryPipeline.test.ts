...
import {
  ...
  type QueryDeps
} from '../../src/core/query/queryPipeline';
  ...
  return {
    async generateStructured<T>(
      ...
      if (req.schemaName === 'query_plan') {
        script.planCalls++;
        if (script.failPlanOnce && script.planCalls === 1)
  ...
  const g: Partial<GraphStore> = {
    ...
    searchDiscussionsByVector: async (_v, _k, filters) => {
      ...
      if (filters.channelIds)
        hits = hits.filter(
          (h) =>
            h.channelId != null && filters.channelIds!.includes(h.channelId)
        );
      if (filters.discussionTypes) {
        hits = hits.filter(
          (h) =>
            h.discussionType != null &&
            filters.discussionTypes!.includes(h.discussionType)
        );
      }
      if (filters.since)
        hits = hits.filter(
          (h) => h.startedAt == null || h.startedAt >= filters.since!
        );
      return hits;
    },
...
describe('answerQuestion', () => {
  test('happy path: synthesises an answer with citations from the vector hits', async () => {
    ...
    const graph = makeGraph({
      vectorHits: [match('d1', 0.82, 'negative'), match('d2', 0.55, 'negative')]
    });

    const out = await answerQuestion(
      { question: 'Jaký mají lidé názor na Smarty?', debug: true },
  ...
  test('no evidence: returns the canned low-confidence answer and never calls the synthesiser', async () => {
    const script: LlmScript = { planCalls: 0, synthCalls: 0 };
    const out = await answerQuestion(
      { question: 'Co si myslí komunita o kávovarech?' },
      deps(makeGraph({ vectorHits: [] }), makeLlm(script))
    );
  ...
  test('graph unreachable: bootstrap failure surfaces as GraphUnavailableError', async () => {
    const script: LlmScript = { planCalls: 0, synthCalls: 0 };
    const call = answerQuestion(
      { question: 'cokoliv a něco navíc' },
      deps(makeGraph({ bootstrapThrows: true }), makeLlm(script))
    );
  ...
  test('planner failure: falls back to a raw-question plan and still answers', async () => {
    ...
    const out = await answerQuestion(
      { question: 'Na Linuxu mi nefunguje zvuk po aktualizaci' },
      deps(graph, makeLlm(script))
    );
  ...
  test('planner discussion-type preference is soft: a mismatch does not drop the answer', async () => {
    ...
    const graph = makeGraph({
      vectorHits: [match('d1', 0.9, 'neutral', 'discussion')]
    });

    const out = await answerQuestion(
      { question: 'Na Linuxu nejde zvuk' },
      deps(graph, makeLlm(script, plan))
    );
  ...
  test('request hard filter with no match: relaxes once and notes it', async () => {
    ...
    const out = await answerQuestion(
      {
        question: 'Byla tu nějaká oznámení?',
        filters: { discussionTypes: ['announcement'] },
        debug: true
      },
      deps(graph, makeLlm(script))
    );
  ...
  test('request hard filter with a match: honoured, other types excluded, no note', async () => {
    ...
    const graph = makeGraph({
      vectorHits: [
        match('d1', 0.9, 'neutral', 'announcement'),
        match('d2', 0.85, 'neutral', 'discussion')
      ]
    });
