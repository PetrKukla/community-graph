...
const { createJob, getJob } =
  await import('../../src/db/sqlite/repositories/jobRepository');
...
describe('recoverInterruptedJobs', () => {
  ...
  test('persists params and reaches a terminal state for a stuck pipeline job', async () => {
    const id = createJob('pipeline', 'chan-x', {
      options: { skipGraphWrite: true, maxDiscussions: 3 }
    });
    expect(getJob(id)?.params).toEqual({
      options: { skipGraphWrite: true, maxDiscussions: 3 }
    });
    forceStatus(id, 'running');
