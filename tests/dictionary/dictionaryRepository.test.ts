...
describe("syncDictionary", () => {
  test("creates, updates and reports unchanged per section", () => {
    ...
    expect(first.channels).toEqual({ received: 1, created: 1, updated: 0, unchanged: 0 });
    expect(first.users).toEqual({ received: 2, created: 2, updated: 0, unchanged: 0 });
    expect(first.changedIds).toEqual({ guildId: "g1", channelIds: ["c1"], userIds: ["u1", "u2"] });
    ...
    expect(second.channels).toEqual({ received: 1, created: 0, updated: 0, unchanged: 1 });
    expect(second.users).toEqual({ received: 2, created: 0, updated: 0, unchanged: 2 });
    expect(second.changedIds).toEqual({ guildId: null, channelIds: [], userIds: [] });

    // a real rename shows as updated
    const third = syncDictionary({ users: [{ id: "u1", display_name: "Alice B." }] });
    expect(third.users).toEqual({ received: 1, created: 0, updated: 1, unchanged: 0 });
    expect(third.changedIds.userIds).toEqual(["u1"]);
    expect(db.select().from(users).where(eq(users.id, "u1")).get()?.displayName).toBe("Alice B.");
  });

  test("null clears a name, an absent key leaves it untouched", () => {
    syncDictionary({ users: [{ id: "u3", username: "carol", display_name: "Carol" }] });
  ...
  test("a section-scoped sync does not touch other sections", () => {
    syncDictionary({ guild: { id: "g2", name: "Another" }, channels: [{ id: "c9", name: "x" }] });
    const before = db.select().from(guilds).where(eq(guilds.id, "g2")).get()?.namesSyncedAt;

    syncDictionary({ users: [{ id: "u9", username: "z" }] });
    const after = db.select().from(guilds).where(eq(guilds.id, "g2")).get()?.namesSyncedAt;
    expect(after).toBe(before);
    expect(db.select().from(channels).where(eq(channels.id, "c9")).get()?.name).toBe("x");
  });
});
