...
const { discussionsLocal, discussionEnrichment, messages } = await import("../../src/db/sqlite/schema");
...
db.insert(messages)
  .values([
    { id: "m1", channelId: "c1", authorId: "u1", content: "nejde zvuk", createdAt: "2026-08-01T00:10:00.000Z", wordCount: 2, ingestedAt: "x", discussionId: "d1" },
    { id: "m2", channelId: "c1", authorId: "u2", content: "zkus restart pipewire", createdAt: "2026-08-01T00:20:00.000Z", wordCount: 3, ingestedAt: "x", discussionId: "d1" },
  ])
  .run();
...
describe("web query view read endpoints", () => {
  ...
  test("GET /graph/node/by-domain-id is registered (not 404-as-unrouted)", async () => {
    const res = await app.request("/api/v1/graph/node/by-domain-id?label=Discussion&id=d1", auth);
