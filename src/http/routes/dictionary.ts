...
const bodySchema = z
  .object({
    guild: z
      .object({ id: z.string().min(1), name: nullableName })
      .strict()
      .optional(),
    channels: z
      .array(
    ...
    users: z
      .array(
        z
          .object({
            id: z.string().min(1),
            username: nullableName,
            display_name: nullableName
          })
          .strict()
      )
      .optional()
    ...
    return c.json(
      {
        ...
        details: 'aspoň jedna ze sekcí guild/channels/users musí být neprázdná'
      },
      400
    );
  }
 ...
 */
async function propagateNames(
  changed: DictionaryChangedIds
): Promise<GraphPropagation> {
  const changedCount =
    (changed.guildId ? 1 : 0) +
    changed.channelIds.length +
    changed.userIds.length;
  const configured = isNeo4jConfigured();
  if (!configured || changedCount === 0)
    return { configured, propagated: false };
  ...
  try {
    ...
    console.error(
      `[dictionary] Neo4j propagation failed: ${err instanceof Error ? err.message : String(err)}`
    );
