...
const withBatchSchema = ingestBatchSchema.extend({
  options: optionsSchema.optional()
});
const channelOnlySchema = z
...
function toChannelOptions(
  opts: z.infer<typeof optionsSchema> | undefined
): PipelineChannelOptions {
  return {
    maxMessages: opts?.max_messages,
    maxDiscussions: opts?.max_discussions,
    skipGraphWrite:
      opts?.skip_graph_write ?? !config.pipeline.include_graph_write
  };
}
