...
import { answerQuestion, GraphUnavailableError } from "../../core/query/queryPipeline";
...
queryRoute.post("/query", async (c) => {
  ...
  const debug = ["1", "true", "yes"].includes((c.req.query("debug") ?? "").toLowerCase());
  ...
  try {
    ...
    console.error(`[query] failed: ${err instanceof Error ? err.stack ?? err.message : String(err)}`);
    return c.json({ error: "query_failed", detail: err instanceof Error ? err.message : String(err) }, 500);
  }
});
