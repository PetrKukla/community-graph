  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (arg) {
      ...
      case '--channel': {
        ...
        if (eq === -1)
          throw new Error(`--channel čekám tvar id=name, dostal jsem: ${raw}`);
      ...
      case '-h':
        ...
        break;
      default:
        if (arg.startsWith('-')) throw new Error(`neznámá volba: ${arg}`);
        if (parsed.file)
          throw new Error(`čekám jen jeden vstupní soubor, druhý: ${arg}`);
        parsed.file = arg;
    }
  }
