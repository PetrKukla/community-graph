import { getInterruptedJobs, markJobFailed } from "../db/sqlite/repositories/jobRepository";
import type { DictionaryNames } from "../core/ports/GraphStore";
import type { PipelineChannelOptions } from "./pipelineStage";
import { runClusterJob, runEnrichJob, runGraphWriteJob, runNameSyncJob, runPipelineJob } from "./jobRunner";
  ...
  for (const job of stuck) {
    ...
    try {
      switch (job.type) {
        ...
        case "enrich":
          requireChannel(job.channelId);
          runEnrichJob(job.id, job.channelId, { maxDiscussions: numberOrUndefined(params.maxDiscussions) });
          requeued++;
          break;
        case "graph_write":
          requireChannel(job.channelId);
          runGraphWriteJob(job.id, job.channelId, { maxDiscussions: numberOrUndefined(params.maxDiscussions) });
          requeued++;
          break;
        case "pipeline":
          requireChannel(job.channelId);
          runPipelineJob(job.id, job.channelId, undefined, (params.options as PipelineChannelOptions | undefined) ?? {});
          requeued++;
          break;
        case "name_sync":
          if (params.names) {
            ...
            markJobFailed(job.id, "přerušeno restartem aplikace; spusť POST /api/v1/dictionary/graph-resync");
            dropped++;
          }
          break;
        default:
          markJobFailed(job.id, `přerušeno restartem aplikace (neznámý typ '${job.type}')`);
          dropped++;
      }
    } catch (err) {
      markJobFailed(job.id, `obnova po restartu selhala: ${err instanceof Error ? err.message : String(err)}`);
      dropped++;
    }
  }

  console.log(`[jobs] po restartu: ${requeued} jobů znovu spuštěno, ${dropped} označeno jako failed`);
}
