...
import {
  graphWriteChannel,
  type GraphWriteChannelOptions
} from './graphWriteStage';
import { nameSyncGraph } from './nameSyncStage';
import {
  executePipeline,
  type PipelineChannelOptions,
  type PipelineResult
} from './pipelineStage';
import {
  markJobRunning,
...
export function runClusterJob(jobId: string, channelId: string): void {
  ...
  clusterChannel(channelId, embeddingProvider)
    ...
    .catch((err) =>
      markJobFailed(jobId, err instanceof Error ? err.message : String(err))
    );
}

export function runEnrichJob(
  jobId: string,
  channelId: string,
  ...
  llmCallContext
    .run({ jobId, channelId }, () =>
      enrichChannel(channelId, getLLMProvider(), embeddingProvider, options)
    )
    .then((result) => markJobCompleted(jobId, { ...result }))
    .catch((err) =>
      markJobFailed(jobId, err instanceof Error ? err.message : String(err))
...
export function runGraphWriteJob(
  jobId: string,
  channelId: string,
...
export function runPipelineJob(
  ...
  const stages = {
    cluster: (id: string, o: { maxMessages?: number }) =>
      clusterChannel(id, embeddingProvider, o),
    enrich: (id: string, o: { maxDiscussions?: number }) =>
      llmCallContext.run({ jobId, channelId: id }, () =>
        enrichChannel(id, getLLMProvider(), embeddingProvider, o)
      ),
  ...
  executePipeline(channelId, stages, options, seed, {
    ...
    .then((result) =>
      markJobCompleted(jobId, result as Record<string, unknown>)
    )
    .catch((err) =>
      markJobFailed(jobId, err instanceof Error ? err.message : String(err))
    );
