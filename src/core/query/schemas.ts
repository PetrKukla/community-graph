...
export const queryPlanSchema = z.object({
  ...
  topics: z
    .array(z.string())
    .describe(
      'Kandidátní kanonické názvy témat k dohledání mezi Topic uzly. Preferuj názvy ze slovníku grafu, pokud sedí.'
    ),
  entities: z
    .array(z.string())
    .describe(
      'Kandidátní pojmenované entity (produkt, technologie, značka, osoba, místo). Prázdné, pokud žádné.'
    ),
  ...
  answer_language: z
    .string()
    .describe(
      "ISO 639-1 kód jazyka, ve kterém je otázka a ve kterém se má odpovědět (např. 'cs', 'en')."
    )
});
...
export const answerSchema = z.object({
  ...
  used_citations: z
    .array(z.string())
    .describe(
      "Seznam identifikátorů diskuzí použitých v odpovědi, přesně jak jsou v kontextu, např. ['D1','D3']."
    ),
  confidence: z
    .enum(['high', 'medium', 'low'])
    .describe(
      'high = podklady přímo a jednoznačně odpovídají; medium = částečně; low = jen okrajově / nejistě.'
    ),
  caveats: z
    .string()
    .nullable()
    .describe(
      'Krátce co podklady nepokrývají nebo kde je odpověď nejistá. null, pokud není co dodat.'
    )
});
