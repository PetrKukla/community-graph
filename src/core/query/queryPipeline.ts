...
export async function answerQuestion(
  ...
  const vocab = await deps.graph
    .sampleLabelVocab(cfg.vocab_sample_size)
    .catch(() => ({ topics: [], entities: [] }));
  ...
  try {
    ...
    console.error(
      `[query] planner failed, using fallback: ${err instanceof Error ? err.message : String(err)}`
    );
  ...
  const runRetrieval = async (f: RetrievalFilters, suffix: string) => {
    const { questionVector, candidates } = await clock(
      `retrieve${suffix}`,
      retrieve(req.question, plan, f, deps, cfg)
    );
    await clock(
      `expand${suffix}`,
      expand(candidates, questionVector, deps, cfg)
    );
  ...
  if (evidence.length === 0 && (filters.discussionTypes || filters.since)) {
    const relaxed: RetrievalFilters = {
      channelIds: filters.channelIds,
      discussionTypes: null,
      since: null
    };
  ...
  if (evidence.length === 0) {
    ...
    if (req.debug)
      answer.debug = buildDebug(plan, filters, ranked, evidenceIds, timings);
    return answer;
  }

  const { contextText, items } = await clock(
    'context',
    buildContext(evidence, deps, cfg)
  );
  const draft = await clock(
    'synthesize',
    synthesizeAnswer(req.question, plan, contextText, deps.llm)
  );
  ...
  const notes = [relaxedNote, draft.caveats].filter((s): s is string =>
    Boolean(s)
  );
