...
export async function planQuery(question: string, llm: LLMProvider, vocab: LabelVocab): Promise<QueryPlan> {
  ...
  if (value.search_queries.length === 0) value.search_queries = [question.trim()].filter(Boolean);
  return value;
}
