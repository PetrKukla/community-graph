...
export const enrichmentSegmentSchema = z.object({
  ...
  title: z.string().describe("Short human title of the discussion (in the discussion's own language)."),
  summary: z.string().describe("A few sentences capturing what was actually said and concluded."),
  topics: z.array(z.string()).describe("Canonical short topic phrases, e.g. 'graphics cards', 'Arch Linux audio'."),
  entities: z
    .array(z.object({ name: z.string(), type: z.string() }))
    .describe("Named things: type is one of person|product|technology|organization|place|event|other."),
  key_points: z.array(z.string()).describe("Bullet-style facts, opinions or decisions worth remembering."),
  sentiment: z.enum(SENTIMENTS),
  sentiment_score: z.number().min(-1).max(1).describe("-1 very negative, 0 neutral, 1 very positive."),
  language: z.string().describe("Dominant language of the discussion as an ISO 639-1 code, e.g. 'cs', 'en'."),
  discussion_type: z.enum(DISCUSSION_TYPES),
  resolved: z
    .boolean()
    .nullable()
    .describe("For question/help-request: true if resolved in-thread, false if not, null if N/A."),
});
